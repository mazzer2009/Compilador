...
#   linguagem t++
# Autores: Emanuel Mazzer
#-------------------------------------------------------------------------

from ply import yacc
from analise import Lexer
...
class Parser:
    ...
    def p_declaracao_variaveis(self, p):
    	'declaracao_variaveis : tipo POINT lista_variaveis'
    ...
    def p_lista_variaveis(self, p):
    	'lista_variaveis : lista_variaveis COMMA var'
    ...
    def p_indice(self, p):
    	'indice : indice LCOLC expressao RCOLC'
    	p[0] = Tree('indice',[p[1],p[3]])

    def p_indice1(self, p):
    	'indice : LCOLC expressao RCOLC'
    	p[0] = Tree('indice',[p[2]])
    ...
    def p_cabecalho(self, p):
    	'cabecalho : ID LPAR lista_parametros RPAR corpo END'
    ...
    def p_parametro(self, p):
    	'parametro : tipo POINT ID'
    	p[0] = Tree('parametro',[p[1]],p[3])


    def p_parametro1(self, p):
    	'parametro : parametro LCOLC RCOLC'
    ...
    def p_se(self, p):
    	'se : IF expressao THEN corpo END '
    	p[0] = Tree('se', [p[2],p[4]])



    def p_se1(self, p):
    	'se : IF expressao THEN corpo ELSE corpo END'
    	p[0] = Tree('se',[p[2],p[4],p[6]])


    def p_repita(self, p):
    	'repita : REPEAT corpo UNTIL expressao'
    ...
    def p_leia(self, p):
    	'leia : READ LPAR ID RPAR'
    	p[0] = Tree('leia',[],p[3])


    def p_escreva(self, p):
    	'escreva : WRITE LPAR expressao RPAR'
    	p[0] = Tree('escreva',[p[3]])


    def p_retorna(self, p):
    	'retorna : RETURN LPAR expressao RPAR'
